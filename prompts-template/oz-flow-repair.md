...
- `pre_qa_audit`（`audit_N`）：全量检查 acceptance、完整 diff、源码和测试；修复后继续自查，只有零新问题且 required tests 通过才可移交 QA。
- `qa_targeted_repair`（`targeted_repair_N`）：仅处理最新 QA findings、失败验收项及直接回归，不得扩审。
