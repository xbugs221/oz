...
- **当** sealed run 正在执行当前 change
- **且** 用户在工作区新增或追加 `docs/changes/<非当前 change>/` 下的需求提案内容
- **则** 当前 run 不得中止
...
- **当** 归档前提升提交级证据时发现缺文件、内容过少或格式不合格等智能体可修复问题
- **则** 系统必须把门禁错误注入同一 repairer 会话并自动进入新的全量 audit，修复后重新运行独立 QA，不得立即进入人工暂停
- **且** 此类确定性修复不得因全量自查轮次上限被跳过
- **当** 修复后再次出现相同门禁阻碍
- **则** 系统才可判定智能体无法自行修复并进入 `blocked_stalled`；若阻碍指纹变化则继续自修
