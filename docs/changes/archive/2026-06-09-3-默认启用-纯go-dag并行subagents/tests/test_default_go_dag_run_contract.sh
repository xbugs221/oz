#!/usr/bin/env bash
# 文件目的：通过真实 wo run/status 入口验证默认执行使用内嵌 go-dag engine，而不是 Dagu CLI。
set -euo pipefail
...
cat >"$fakebin/dagu" <<'SH'
#!/usr/bin/env bash
# 这个 fake dagu 代表用户机器上不可用或失败的 Dagu CLI；默认 go-dag 不应触碰它。
set -euo pipefail
...
note "运行默认 wo run，PATH 中的 dagu 会失败，因此成功代表默认路径没有依赖 Dagu CLI"
...
test ! -e "$tmp/dagu.called" || fail "默认 go-dag run 不得调用 Dagu CLI"
