...
- [x] 2.1 删除旧后端 runner、参数构造、JSONL session 解析、集成测试和单元测试。
- [x] 2.2 收窄 agent registry 和配置校验，只支持 `codex`、`pi`。
- [x] 2.3 删除状态展示、恢复、session 隔离和测试夹具中的旧后端 key。
- [x] 2.4 清理帮助文案、配置示例、profiles、主规格、长期测试和归档材料中的旧后端描述。

## 3. 增加启动前 CLI 检查

- [x] 3.1 sealed run 创建运行态前检查 `codex` 和 `pi` 是否都在 PATH 中。
- [x] 3.2 任一缺失时输出明确安装提示。
- [x] 3.3 缺失 CLI 时不得创建或污染 run state。
...
- [x] 5.1 重新运行本提案全部契约测试。
- [x] 5.2 运行受影响的根目录长期测试。
- [x] 5.3 更新 `docs/specs/codex-workflow-cli/spec.md`。
- [x] 5.4 确认 `acceptance.json` 中 required tests 和 evidence 均有真实输出。
