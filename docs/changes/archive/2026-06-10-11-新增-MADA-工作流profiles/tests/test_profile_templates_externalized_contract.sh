#!/usr/bin/env bash
# 文件功能目的：验证 wo 默认工作流和 MADA profiles 的 subagent/prompt 配置已从 Go 硬编码迁移到内置 YAML 模板。
set -euo pipefail
...
note() {
  # note 记录模板抽离合同的每个真实检查步骤，便于 review/QA 复核。
  printf '%s\n' "$*" | tee -a "$log"
}
...
assert_file() {
  # assert_file 确认内置 profile 模板已经成为真实仓库文件。
  local path="$1"
  [[ -f "$path" ]] || fail "缺少模板文件: $path"
}

assert_contains() {
  # assert_contains 验证模板或生成产物保留必要的业务语义。
  local file="$1"
  local text="$2"
...
assert_production_go_not_contains() {
  # assert_production_go_not_contains 防止默认 subagent 描述继续留在生产 Go 字符串中。
  local text="$1"
  local file
...
new_repo() {
  # new_repo 创建真实 git 仓库，用默认 wo config 路径验证模板输出仍可加载。
