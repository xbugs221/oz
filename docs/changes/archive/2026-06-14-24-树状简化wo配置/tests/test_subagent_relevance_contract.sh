...
result_dir="$repo_root/test-results/24-树状简化wo配置"
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
...
if stage == "execution":
    ...
    (run_dir / "review-1.json").write_text(json.dumps({
        ...
        "evidence": [
            "go test runtime evidence: test-results/24-树状简化wo配置/subagent-relevance.log",
    ...
    (run_dir / "qa-1.json").write_text(json.dumps({
        ...
        "decision": "clean",
        "evidence": ["runtime log: test-results/24-树状简化wo配置/subagent-relevance.log"],
        "findings": [],
        "acceptance_matrix": [
            {
                ...
                "evidence": "test-results/24-树状简化wo配置/subagent-relevance.log"
            }
        ]
    }, ensure_ascii=False), encoding="utf-8")
elif stage == "archive":
