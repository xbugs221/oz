#!/usr/bin/env bash
# 文件功能目的：用真实伪 TTY 验证 wo watch 在长中文提案名换行后不会残留旧帧首行。
set -euo pipefail

repo_root="$(git rev-parse --show-toplevel)"
result_dir="$repo_root/test-results/9-status-watch-proposal-list"
log="$result_dir/watch-tty-clear-long-change.log"
tmpdir="$(mktemp -d)"
...
note() {
  # note 记录伪 TTY 测试步骤，便于复查真实终端刷新问题。
  printf '%s\n' "$*" | tee -a "$log"
}

require_script() {
  # require_script 保证本测试运行在能够分配伪 TTY 的系统上。
...
note "创建临时 git 仓库和真实 runtime state"
mkdir -p "$repo"
cd "$repo"
git init >/dev/null
...
write_json(runs / run_id / "state.json", {
    "run_id": run_id,
    "change_name": change,
    "sealed": True,
    "status": "running",
...
write_json(batches / batch_id / "state.json", {
    "batch_id": batch_id,
    "status": "running",
    "changes": [change],
    "current_index": 0,
