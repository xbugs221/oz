...
log="$repo_root/test-results/oz-flow-merge/no-wo-legacy-surface-contract.log"
mkdir -p "$(dirname "$log")"
: >"$log"
...
note "evidence id: no-wo-legacy-surface-log"
note "evidence path: $log"
note "test id: no-wo-legacy-surface-contract"

[[ ! -e cmd/wo ]] || fail "仍存在 cmd/wo"

note "检查模板文件名不再使用 wo-*"
if fd '^wo-' prompts-template profiles-template 2>/dev/null | tee -a "$log" | grep -q .; then
  fail "模板文件名仍使用 wo-*"
fi
