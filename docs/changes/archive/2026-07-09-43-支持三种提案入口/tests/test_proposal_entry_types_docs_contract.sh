#!/usr/bin/env bash
# 文件功能目的：验证 README、长期规格和内置 oz skills 已说明 micro、small、standard 三种提案入口。
set -euo pipefail

ROOT="$(git rev-parse --show-toplevel)"
LOG="$ROOT/test-results/43-proposal-entry-types/docs-contract.log"
...
fail() {
  # 函数目的：用明确业务原因终止测试，避免只暴露 grep 失败。
  note "FAIL: $*"
  exit 1
}
...
note "PASS: proposal entry type docs contract"
