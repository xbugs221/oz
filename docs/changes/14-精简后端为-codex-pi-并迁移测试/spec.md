...
- **对应测试**：`docs/changes/14-精简后端为-codex-pi-并迁移测试/tests/test_backend_removal_contract.sh`
- **真实数据来源**：当前仓库真实文件树
- **入口路径**：仓库根目录
...
长期测试必须位于仓库根目录 `tests/` 下，`internal/` 只保留生产源码。执行阶段可以重写测试形态，但不得降低业务覆盖。
...
- **当** 扫描 `internal/` 目录
- **则** 不存在 `*_test.go`
- **并且** 根目录 `tests/` 下存在可运行的 Go 或 shell 业务测试入口
- **对应测试**：`docs/changes/14-精简后端为-codex-pi-并迁移测试/tests/test_root_test_layout_contract.sh`
- **真实数据来源**：当前仓库真实文件树
- **入口路径**：仓库根目录
...
- **则** 文档只声明 `codex` 和 `pi`
- **且** 文档说明启动前检查两个 CLI
- **且** 发布门禁使用根目录 `tests/` 作为长期测试入口
- **对应测试**：`docs/changes/14-精简后端为-codex-pi-并迁移测试/tests/test_docs_release_gate_contract.sh`
