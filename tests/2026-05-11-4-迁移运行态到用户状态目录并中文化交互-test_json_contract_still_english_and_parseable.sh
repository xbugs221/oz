#!/usr/bin/env bash
# Build wo and verify JSON runner output keeps English fields and enum values.
set -euo pipefail
...
HOME="$home" XDG_STATE_HOME="$state_home" "$bin" status --run-id "$run_id" --json > status.json
grep -q '"run_id":"'"$run_id"'"' status.json
grep -q '"status":"done"' status.json
grep -q '"stage":"done"' status.json
grep -q '"stages":' status.json
grep -q '"paths":' status.json
grep -q '"sessions":' status.json
grep -q '"error":' status.json
