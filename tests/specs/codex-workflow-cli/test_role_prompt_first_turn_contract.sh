...
// TestBundledReviewPromptKeepsExamplesOnlyForFirstReviewerTurn verifies reviewer history carries examples after startup.
func TestBundledReviewPromptKeepsExamplesOnlyForFirstReviewerTurn(t *testing.T) {
	...
	requirePromptContains(t, first, "JSON schema：", "如需修复，使用：", "如需提前终止无效循环，使用：", "review-1.json")
	...
	requirePromptContains(t, resumed, "review-2.json", "review-1.json", "fix-1-summary.md", "只输出一个 JSON 对象")
	requirePromptOmits(t, resumed, "JSON schema：", "如需修复，使用：", "如需提前终止无效循环，使用：", "\"summary\": \"一句话总结审核结果\"", "\"decision\": \"needs_fix\"")
}

// TestBundledQAPromptKeepsExamplesOnlyForFirstQATurn verifies QA examples are not replayed after session resume.
func TestBundledQAPromptKeepsExamplesOnlyForFirstQATurn(t *testing.T) {
	...
	requirePromptContains(t, first, "clean 示例：", "needs_fix 示例：", "acceptance_matrix")

	resumed := renderBundledPromptForRoleContract(t, "wo-qa.md", "wo-qa", "qa_2", map[string]string{"codex:qa": "qa-session"})
	requirePromptContains(t, resumed, "qa-2.json", "acceptance_matrix", "只输出一个 JSON 对象")
