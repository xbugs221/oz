#!/usr/bin/env bash
# Sources: 29-拆分工作流配置解析边界
# 文件功能目的：验证工作流配置 schema、profile、parallel 和 validation 解析保持稳定边界。
set -euo pipefail

repo_root="$(git rev-parse --show-toplevel)"
result_dir="$repo_root/test-results/codex-workflow-cli/workflow-config-boundary"
log="$result_dir/workflow-config-boundary.log"
mkdir -p "$result_dir"
: >"$log"
