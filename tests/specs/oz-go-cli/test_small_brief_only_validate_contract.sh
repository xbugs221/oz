#!/usr/bin/env bash
# 文件功能目的：长期验证 oz validate 接受 small brief-only 提案，同时保留 acceptance 和 tests 硬合同。
# Sources: 43-支持三种提案入口
set -euo pipefail

ROOT="$(git rev-parse --show-toplevel)"
RESULT_DIR="$ROOT/test-results/specs/oz-go-cli"
LOG="$RESULT_DIR/small-brief-only-validate-contract.log"
OZ_BIN="$RESULT_DIR/oz-small-brief-only"
TMP="$(mktemp -d)"

mkdir -p "$RESULT_DIR"
: >"$LOG"
trap 'rm -rf "$TMP" "$OZ_BIN"' EXIT
