...
- 把 `.gitignore` 视为提交硬边界：只用明确路径逐文件暂存，不得使用 `git add .`、`git add -A` 或 `git add -f`
- 提交前运行 `git diff --cached --name-only` 获取完整暂存清单，并对每个候选路径运行 `git check-ignore --no-index -- <path>`；命中忽略规则的路径一律从暂存区移除，即使它与当前提案相关、曾被跟踪或因合并草稿进入暂存区
...
- 受影响规格测试已经重新运行
- 暂存清单已经逐项复核，且不包含任何被 `.gitignore` 命中的路径
...
| “忽略文件与当前提案相关，可以强制提交” | `.gitignore` 是硬边界；移出暂存区，禁止用 `git add -f` 绕过 |
