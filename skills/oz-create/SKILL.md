...
创建 `oz` 变更提案并生成所有产物。不要只写四个 Markdown 文件；`tests/` 和 `acceptance.json` 也是提案结构的一部分，必须在创建阶段落盘，并由创建阶段锁定用户真实意图对应的契约测试和验收证据合同。

- `proposal.md`：说明做什么和为什么
- `design.md`：说明关键技术决策、取舍和风险
- `spec.md`：使用中文 `### 需求：` 和 `#### 场景：` 描述验收行为
- `task.md`：拆分实现步骤和验证条件
- `acceptance.json`：结构化验收合同，列出后续 wo QA 必须逐项证明的测试和证据
- `tests/`：必须包含真实项目测试代码，用来表达本次变更的核心契约；不得为空，不写测试说明文档或占位文件
...
- `design.md` 说明 `tests/` 中每个测试文件覆盖的核心业务契约、真实数据来源、入口路径、断言和剩余风险
- `task.md` 的第一组任务必须是先运行创建阶段写好的契约测试；如果功能尚未实现，预期应因目标行为缺失而失败，不得因测试语法、路径或环境配置错误失败
- `tests/` 中写入可执行的真实项目测试代码，优先沿用仓库已有测试框架和目录风格
- `acceptance.json` 必须复用 `tests/` 中的契约测试，并补齐根目录端到端/回归测试、截图、trace、network、console、runtime log 等 QA 证据要求
