...
然后运行 `oz validate <change> --json` 检验，确认 `tests/` 目录存在、包含测试代码且没有占位内容；同时人工核对 `acceptance.json` 是合法 JSON，且其中的 `coverage` / `required_tests` / `required_evidence` 与文档、测试路径和测试脚本内部断言一致。
