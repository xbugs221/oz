...
创建 `oz` 变更提案并生成所有产物。不要只写四个 Markdown 文件；`tests/` 也是提案结构的一部分，必须在创建阶段落盘，并由创建阶段锁定用户真实意图对应的契约测试骨架。
...
创建阶段负责定义核心契约测试，执行阶段负责让这些测试通过。不得把核心行为标准推迟给执行器自行制定。

如果当前工作流还有独立 acceptance 阶段，职责边界如下：

- `oz-create` 的 `tests/` 负责 proposal/design/spec 的契约测试骨架，优先放在 `docs/changes/<change>/tests/`
- acceptance 阶段负责复用这些契约测试，并补齐根目录端到端/回归测试、截图、trace、network、runtime log 等证据合同
- 不要因为 acceptance 会补端到端验收，就省略 `docs/changes/<change>/tests/`
...
如果暂时无法确定测试策略、真实数据来源或用户可感知断言，先和用户澄清；不要创建缺少契约测试的提案。

执行阶段可以新增单元测试、回归测试、端到端测试或边界测试，但不得删除、弱化或绕过创建阶段写入的契约测试。只有用户明确变更意图时，才能同步更新 `spec.md`、`design.md`、`task.md` 和 `tests/`，并记录原因。
